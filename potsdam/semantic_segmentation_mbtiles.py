...
def build_scene(task, data_uri, id, channel_order=None):
    ...
    mbtiles_uri = '/opt/data/raw-data/germany.mbtiles'
    class_id_to_filter = {
        1: ['has', 'building']
    }
    vector_source = rv.VectorSourceConfig.builder(rv.MBTILES_SOURCE) \
        .with_uri(mbtiles_uri) \
        .with_class_filters(class_id_to_filter) \
        .build()
    background_class_id = 2
    rasterized_source = rv.RasterSourceConfig.builder(rv.RASTERIZED_SOURCE) \
        .with_vector_source(vector_source) \
        .with_rasterizer_options(background_class_id) \
        .build()
    label_source = rv.LabelSourceConfig.builder(rv.SEMANTIC_SEGMENTATION) \
        .with_raster_source(rasterized_source) \
        .build()
